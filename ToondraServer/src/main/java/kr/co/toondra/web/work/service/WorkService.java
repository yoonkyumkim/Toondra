...
public class WorkService {
	...
	public PMap insertWork(PMap pMap) {
		...
		if(mFile.getSize() > 0) {
			String workSeq = dao.getWorkSeq().get("work_seq").toString();
			pMap.put("work_seq", workSeq);
			String filePath = ROOT_PATH + "/" + workSeq + "/" + fileName;
			File workDir = new File(ROOT_PATH + "/" +workSeq);
			workDir.mkdir();
	...
	public PMap updateWork(PMap pMap) {
		...
		String addAuthorList[] = pMap.getRequest().getParameterValues("author_seq");
		String addGenreList[] = pMap.getRequest().getParameterValues("genre_seq");
		String delAuthorList[] = pMap.getRequest().getParameterValues("del_author");
		String delGenreList[] = pMap.getRequest().getParameterValues("del_genre");
		...
		String beforethumbnailFilePath = ROOT_PATH + "/" +pMap.get("work_seq")+ "/" + beforeThumbnail;
		if(mFile.getSize() > 0) {
			File file = new File(filePath);
			File dir = new File(ROOT_PATH + "/" +pMap.get("work_seq"));
			if(!dir.exists()) {
				dir.mkdir();
			}
			File before_file = new File(beforethumbnailFilePath);
			before_file.delete();
			try {
				mFile.transferTo(file);
			} catch (IllegalStateException e) {
